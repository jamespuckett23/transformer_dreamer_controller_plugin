...
def load_next_map():
	# this may or may not be possible, but I want to be able to load the next gz map.
  # this may require setting up a ros2-gz msg bridge that you can send a message to the gz environment
	return
...
def train(model, cfg, device):
  ...
  # train single simulation map
  episodes = [] # TODO episodes lists start and goal locations to train through
	load_next_map()

  train_env = 0 # TODO train_env is the nav2/gz/gym environment for this specific map
  global_step = 0
  ...
  temp = cfg.arch.world_model.temp_start
	...
	for _ in range(cfg.total_steps):
		...
		traj = model.world_model_imagine() # TODO maybe imagine_ahead
